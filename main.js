...
/* =========================================
   5. 3D BORDER GLOW CURSOR TRACKING
========================================= */
document.addEventListener("DOMContentLoaded", () => {
    const glowCards = document.querySelectorAll('.border-glow-card');
    
    // Exact colors from your React snippet (Purple, Pink, Blue)
    const colors = ['#c084fc', '#f472b6', '#38bdf8']; 
    const glowColor = '40deg 80% 80%';
    const intensity = 1;

    glowCards.forEach(card => {
        // Pre-calculate gradient vars for this card
        const gradientPositions = ['80% 55%', '69% 34%', '8% 6%', '41% 38%', '86% 85%', '82% 18%', '51% 4%'];
        const colorMap = [0, 1, 2, 0, 1, 2, 1];
        
        for (let i = 0; i < 7; i++) {
            const c = colors[colorMap[i]];
            card.style.setProperty(`--gradient-${['one', 'two', 'three', 'four', 'five', 'six', 'seven'][i]}`, `radial-gradient(at ${gradientPositions[i]}, ${c} 0px, transparent 50%)`);
        }
        card.style.setProperty('--gradient-base', `linear-gradient(${colors[0]} 0 100%)`);

        // Apply Glow opacities based on HSL setup
        const opacities = [100, 60, 50, 40, 30, 20, 10];
        const keys = ['', '-60', '-50', '-40', '-30', '-20', '-10'];
        for (let i = 0; i < opacities.length; i++) {
            card.style.setProperty(`--glow-color${keys[i]}`, `hsl(${glowColor} / ${Math.min(opacities[i] * intensity, 100)}%)`);
        }
