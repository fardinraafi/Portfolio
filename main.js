   ...
   // C. 3D Magnetic Hover Effect for ALL Cards
    ...
    magneticCards.forEach(card => {
        ...
        card.addEventListener('mouseleave', () => {
           ...
           /* =========================================
       12. PRO AJAX FORM SUBMISSION (WEB3FORMS)
    ========================================= */
    const web3Forms = document.querySelectorAll('form[action="https://api.web3forms.com/submit"]');
    
    web3Forms.forEach(form => {
        form.addEventListener('submit', function(e) {
            e.preventDefault(); // This stops the browser from going to the ugly white Web3Forms page!
            
            const submitBtn = form.querySelector('button[type="submit"]');
            const originalBtnText = submitBtn.innerHTML;
            submitBtn.innerHTML = 'Sending...'; // Changes button text so user knows it's working
            
            const formData = new FormData(form);
            const redirectUrl = formData.get('redirect'); // Grabs the secret redirect link you set in HTML
            
            // Format data for Web3Forms background API
            ...
            .then(async (response) => {
                if (response.status === 200) {
                    // Success! Now WE control the redirect, not Web3Forms.
                    if (redirectUrl) {
